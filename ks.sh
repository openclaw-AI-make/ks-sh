...
canshu_v6() {
	if grep -q '^canshu="V6"' /usr/local/bin/ks > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/ks.sh
	fi
}
...
canshu_v6yinsiyuanquan2


sed -i '/^alias ks=/d' ~/.bashrc > /dev/null 2>&1
sed -i '/^alias ks=/d' ~/.profile > /dev/null 2>&1
sed -i '/^alias ks=/d' ~/.bash_profile > /dev/null 2>&1
cp -f ./ks.sh ~/ks.sh > /dev/null 2>&1
cp -f ~/ks.sh /usr/local/bin/ks > /dev/null 2>&1
ln -sf /usr/local/bin/ks /usr/bin/ks > /dev/null 2>&1
...
check_disk_space() {
	...
	if [ "$available_space_mb" -lt "$required_space_mb" ]; then
		...
		echo "无法继续安装，请清理磁盘空间后重试。"break_end
		ks
	fi
}
...
while true; do
	...
	case $sub_choice in
		1)read -e -p "请输入创建命令: " dockername
			$dockername
			;;
		2)read -e -p "请输入容器名（多个容器名请用空格分隔）: " dockername
			docker start $dockername
			;;
		3)read -e -p "请输入容器名（多个容器名请用空格分隔）: " dockername
			docker stop $dockername
			;;
		4)read -e -p "请输入容器名（多个容器名请用空格分隔）: " dockername
			docker rm -f $dockername
			;;
		5)read -e -p "请输入容器名（多个容器名请用空格分隔）: " dockername
		...
		7)docker stop $(docker ps -q)
			;;
		8)read -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有容器吗？(Y/N): ")" choice
			case "$choice" in
			  [Yy])
		...
		9)docker restart $(docker ps -q)
			;;
		11)read -e -p "请输入容器名: " dockername
			docker exec -it $dockername /bin/sh
			break_end
			;;
		12)read -e -p "请输入容器名: " dockername
		...
		15)read -e -p "请输入容器名: " docker_name
			ip_address
		...
		16)read -e -p "请输入容器名: " docker_name
			ip_address
...
while true; do
	...
	read -e -p "请输入你的选择: " sub_choice
	case $sub_choice in
		1)read -e -p "请输入镜像名（多个镜像名请用空格分隔）: " imagenames
			for name in $imagenames; do
				echo -e "${gl_kjlan}正在获取镜像: $name${gl_bai}"
				docker pull $name
			done
			;;
		2)read -e -p "请输入镜像名（多个镜像名请用空格分隔）: " imagenames
			for name in $imagenames; do
				echo -e "${gl_kjlan}正在更新镜像: $name${gl_bai}"
				docker pull $name
			done
			;;
		3)read -e -p "请输入镜像名（多个镜像名请用空格分隔）: " imagenames
			for name in $imagenames; do
				docker rmi -f $name
			done
			;;
		4)read -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有镜像吗？(Y/N): ")" choice
			case "$choice" in
			  [Yy])
...
install_ldnmp_conf() {
  ...
  wget -O /home/web/nginx.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/nginx10.conf
  wget -O /home/web/conf.d/default.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/default10.conf

  default_server_ssl

  # 下载 docker-compose.yml 文件并进行替换
  wget -O /home/web/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/LNMP-docker-compose-10.yml
  dbrootpasswd=$(openssl rand -base64 16) ; dbuse=$(openssl rand -hex 4) ; dbusepasswd=$(openssl rand -base64 8)

  # 在 docker-compose.yml 文件中进行替换
  sed -i "s#webroot#$dbrootpasswd#g" /home/web/docker-compose.yml
  sed -i "s#ksYYDS#$dbusepasswd#g" /home/web/docker-compose.yml
  sed -i "s#ks#$dbuse#g" /home/web/docker-compose.yml
...
install_ldnmp() {
	  ...
	  # mysql调优
	  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/custom_mysql_config-1.cnf
...
install_certbot() {
	...
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/auto_cert_renewal.sh
...
certs_status() {
	...
	local file_path="/etc/letsencrypt/live/$yuming/fullchain.pem"
	if [ -f "$file_path" ]; thenelseecho -e "${gl_hong}注意: ${gl_bai}证书申请失败，请检查以下可能原因并重试："
...
repeat_add_yuming() {
if [ -e /home/web/conf.d/$yuming.conf ]; thenweb_del "${yuming}" > /dev/null 2>&1
fi
...
nginx_upgrade() {
  ...
  docker restart $ldnmp_pods > /dev/null 2>&1echo "更新${ldnmp_pods}完成"
...
phpmyadmin_upgrade() {
  ...
  docker rm -f $ldnmp_pods > /dev/null 2>&1
  docker images --filter=reference="$ldnmp_pods*" -q | xargs docker rmi > /dev/null 2>&1
  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/refs/heads/main/docker-compose.phpmyadmin.yml
...
web_cache() {cf_purge_cache
  cd /home/web && docker compose restart
}



web_del() {yuming_list="${1:-}"
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  1)
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/fail2ban-nginx-cc.conf
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/config/main/fail2ban/nginx-docker-cc.conf
				  ...
				  21)echo "到cf后台右上角我的个人资料，选择左侧API令牌，获取Global API Key"
					  echo "https://dash.cloudflare.com/login"
					  read -e -p "输入CF的账号: " cfuser
					  read -e -p "输入CF的Global API Key: " cftoken

					  wget -O /home/web/conf.d/default.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/default11.conf
					  docker exec nginx nginx -s reload
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/config/main/fail2ban/cloudflare-docker.conf

					  sed -i "s/ks@outlook.com/$cfuser/g" /etc/fail2ban/action.d/cloudflare-docker.conf
				  ...
				  22)echo -e "${gl_huang}网站每5分钟自动检测，当达检测到高负载会自动开盾，低负载也会自动关闭5秒盾。${gl_bai}"
					  echo "--------------"
					  echo "获取CF参数: "
					  ...
					  install jq bc
					  check_crontab_installed
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/CF-Under-Attack.sh
...
web_optimization() {
		  while true; do
		  	  check_ldnmp_mode
			  check_nginx_compression
			  clearecho -e "优化LDNMP环境${gl_lv}${mode_info}${gzip_status}${br_status}${zstd_status}${gl_bai}"
			  echo "------------------------"
			  ...
			  case $sub_choice in
				  ...
				  # php调优
				  wget -O /home/optimized_php.ini ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/optimized_php.ini
				  docker cp /home/optimized_php.ini php:/usr/local/etc/php/conf.d/optimized_php.ini
				  docker cp /home/optimized_php.ini php74:/usr/local/etc/php/conf.d/optimized_php.ini
				  rm -rf /home/optimized_php.ini

				  # php调优
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/www-1.conf
				  ...
				  # php调优
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/www.conf
				  docker cp /home/www.conf php:/usr/local/etc/php-fpm.d/www.conf
				  docker cp /home/www.conf php74:/usr/local/etc/php-fpm.d/www.conf
				  rm -rf /home/www.conf
				  ...
				  # mysql调优
				  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/custom_mysql_config.cnf
				  docker cp /home/custom_mysql_config.cnf mysql:/etc/mysql/conf.d/
				  rm -rf /home/custom_mysql_config.cnf
...
docker_app() {while true; do
	...
	read -e -p "请输入你的选择: " choice
	 case $choice in
		1)
			...
			while true; do
				...
				if ss -tuln | grep -q ":$app_port "; then
					echo -e "${gl_hong}错误: ${gl_bai}端口 $app_port 已被占用，请更换一个端口"else
		...
		5)
			echo "${docker_name}域名访问设置"add_yuming
		...
		7)clear_container_rules "$docker_name" "$ipv4_address"
			;;

		8)block_container_port "$docker_name" "$ipv4_address"
			;;
...
if [ ! -f "$PROMETHEUS_DIR/prometheus.yml" ]; then
	curl -o "$PROMETHEUS_DIR/prometheus.yml" ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/config/refs/heads/main/prometheus/prometheus.yml
fi
...
f2b_install_sshd() {
	...
	if command -v dnf &>/dev/null; then
		cd /etc/fail2ban/jail.d/
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/config/main/fail2ban/centos-ssh.conf
	fi
...
ldnmp_install_status_one() {
   ...
   if docker inspect "php" &>/dev/null; then
	clearecho -e "${gl_huang}提示: ${gl_bai}建站环境已安装。无需再次安装！"
	break_end
	linux_ldnmp
   fi
...
ldnmp_install_status() {
	...
	if ! docker inspect "php" &>/dev/null; thenldnmp_install_all
	fi
...
nginx_install_status() {
	...
	if ! docker inspect "nginx" &>/dev/null; thennginx_install_all
	fi
...
ldnmp_wp() {
  ...
  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/wordpress.com.conf
  ...
  mkdir $yuming
  cd $yuming
  wget -O latest.zip ${gh_proxy}github.com/openclaw-AI-make/ks-sh/Website_source_code/raw/refs/heads/main/wp-latest.zip
  unzip latest.zip
  rm latest.zip
...
ldnmp_Proxy() {
	...
	wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/reverse-proxy-backend.conf
...
ldnmp_Proxy_backend() {
	clear
	webname="反向代理-负载均衡"echo "开始部署 $webname"
	if [ -z "$yuming" ]; then
		add_yuming
	fi
	...
	while true; do
		...
		case $choice in
			...
			3)
				...
				if [[ "$confirm" =~ ^[Yy]$ ]]; then
					docker rm -f nginx
					sed -i "/\b${app_id}\b/d" /home/docker/appno.txtecho "nginx 容器已删除。"
				else
					echo "操作已取消。"
				fi
			...
			4)
				...
				add_app_id;;
			5)read -e -p "请输入你要编辑的服务名: " stream_name
				install nano
				nano /home/web/stream.d/$stream_name.conf
				docker restart nginx;;
			6)read -e -p "请输入你要删除的服务名: " stream_name
...
ldnmp_Proxy_backend_stream() {
	clear
	webname="Stream四层代理-负载均衡"echo "开始部署 $webname"
	...
	wget -O /home/web/stream.d/$proxy_name.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/reverse-proxy-backend-stream.conf
	...
	while true; do
		...
		case $sub_choice in
			...
			4)echo -e "为现有的站点再关联一个新域名用于访问"
				...
				;;
			5)tail -n 200 /home/web/log/nginx/access.log
				break_end
				;;
			6)tail -n 200 /home/web/log/nginx/error.log
				break_end
				;;
			7)install nano
			...
			8)read -e -p "编辑站点配置，请输入你要编辑的域名: " yuming
				install nano
...
generate_frps_config() {# 生成随机端口和凭证
...
configure_frpc() {read -e -p "请输入外网对接IP: " server_addr
...
add_forwarding_service() {# 提示用户输入服务名称和转发信息
...
delete_forwarding_service() {# 提示用户输入需要删除的服务名称
	...
	while true; do
		...
		case $choice in
			...
			3)
				...
				;;
			5)
				echo "将内网穿透服务反代成域名访问"add_yuming
			...
			7)read -e -p "请输入需要放行的端口: " frps_port
				clear_host_port_rules "$frps_port" "$ipv4_address"
				;;

			8)echo "如果你已经反代域名访问了，可用此功能阻止IP+端口访问，这样更安全。"
...
yt_menu_pro() {
	...
	while true; do
		...
		clearecho -e "yt-dlp $YTDLP_STATUS"
		...
		case $choice in
			...
			7)read -e -p "请输入完整 yt-dlp 参数（不含 yt-dlp）: " custom
...
local CONF="/etc/sysctl.d/99-ks-bbr.conf"
...
sshkey_panel() {
  root_usewhile true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://blog.ks.pro/ssh-key"
  	  echo "------------------------------------------------"
	  ...
	  case $host_dns in
		  ...
		  3)fetch_github_ssh_keys
			break_end
			  ;;
		  4)read -e -p "请输入您的远端公钥URL： " keys_url
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示: ${gl_bai}该功能需要root用户才能运行！" && break_end && ks
}
		...
		dd_xitong_4() {
		  ...
		  while true; do
			...
			case "$sys_choice" in
			  ...
			  42)
				dd_xitong_2bash InstallNET.sh -windows 10 -lang "cn"
				reboot
				exit
				;;
...
bbrv3() {
		  root_uselocal cpu_arch=$(uname -m)
			...
			while true; do
				  ...
				  case $sub_choice in
					  1)
						...
						wget -qO - ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes

						# 步骤3：添加存储库
						echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list

						# version=$(wget -q https://dl.xanmod.org/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
						local version=$(wget -q ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
...
elrepo() {
		  root_useif uname -r | grep -q 'elrepo'; then
			while true; do
				  clear
				  ...
				  case $sub_choice in
					  1)
						...
						elrepo_installserver_reboot

						  ;;
					  2)
						dnf remove -y elrepo-release
						rpm -qa | grep elrepo | grep kernel | xargs rpm -e --nodeps
						echo "elrepo内核已卸载。重启后生效"server_reboot
		  ...
		  root_usewhile true; do
				...
				case $sub_choice in
					...
					2)install_docker
					  ...
					  break_end
						;;
					3)read -e -p "请输入要扫描的目录，用空格分隔（例如：/etc /var /usr /home /root）: " directories
					  install_docker
					  clamav_freshclam
...
_kernel_optimize_core() {
	local mode_name="$1"
	local scene="${2:-high}"
	local CONF="/etc/sysctl.d/99-ks-optimize.conf"
...
SYSCTL
	...
	# ── 文件描述符限制 ──
	if ! grep -q "# ks-optimize" /etc/security/limits.conf 2>/dev/null; then
...
restore_defaults() {
	...
	# 清理文件描述符配置
	if grep -q "# ks-optimize" /etc/security/limits.conf 2>/dev/null; then
		sed -i '/# ks-optimize/,+4d' /etc/security/limits.conf
	fi
...
Kernel_optimize() {
	root_use
	while true; do
	  clearlocal current_mode=$(grep "^# 模式:" /etc/sysctl.d/99-ks-optimize.conf 2>/dev/null | sed 's/# 模式: //' | awk -F'|' '{print $1}' | xargs)
	  ...
	  case $sub_choice in
		  ...
		  6)
			  ...
			  clear
			  restore_defaults
			  curl -sS ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/refs/heads/main/network-optimize.sh -o /tmp/network-optimize.sh && source /tmp/network-optimize.sh && restore_network_defaults;;

		  7)
			  cd ~
			  clear
			  curl -sS ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/refs/heads/main/network-optimize.sh | bash;;
...
linux_trash() {
  root_uselocal bashrc_profile="/root/.bashrc"
...
linux_fav() {bash <(curl -l -s ${gh_proxy}raw.githubusercontent.com/byJoey/cmdbox/refs/heads/main/install.sh)
}

# 创建备份
create_backup() {local TIMESTAMP=$(date +"%Y%m%d%H%M%S")
...
# 删除备份
delete_backup() {read -e -p "请输入要删除的备份文件名: " BACKUP_NAME
...
# 删除连接
delete_connection() {read -e -p "请输入要删除的连接编号: " num
...
# 使用连接
use_connection() {read -e -p "请输入要使用的连接编号: " num
...
ssh_manager() {CONFIG_FILE="$HOME/.ssh_connections"
...
# 持久化挂载分区
mount_partition() {read -e -p "请输入要挂载的分区名称（例如 sda1）: " PARTITION
...
# 卸载分区
unmount_partition() {read -e -p "请输入要卸载的分区名称（例如 sda1）: " PARTITION
...
# 格式化分区
format_partition() {read -e -p "请输入要格式化的分区名称（例如 sda1）: " PARTITION
...
# 检查分区状态
check_partition() {read -e -p "请输入要检查的分区名称（例如 sda1）: " PARTITION
...
# 删除任务
delete_task() {read -e -p "请输入要删除的任务编号: " num
...
run_task() {CONFIG_FILE="$HOME/.rsync_tasks"
...
# 创建定时任务
schedule_task() {read -e -p "请输入要定时同步的任务编号: " num
	if ! [[ "$num" =~ ^[0-9]+$ ]]; then
		echo "错误: 请输入有效的任务编号！"
		return
	fi
...
# 删除定时任务
delete_task_schedule() {read -e -p "请输入要删除的任务编号: " num
	if ! [[ "$num" =~ ^[0-9]+$ ]]; then
		echo "错误: 请输入有效的任务编号！"
		return
	fi
...
linux_info() {
	...
	clear
	echo -e "${gl_kjlan}正在查询系统信息……${gl_bai}"ip_address
...
linux_tools() {
  ...
  while true; do
	  ...
	  case $sub_choice in
		  ...
		  31)
			  clearinstall curl wget sudo socat htop iftop unzip tar tmux ffmpeg btop ranger ncdu fzf cmatrix sl bastet nsnake ninvaders vim nano git
			  ;;

		  32)
			  clearinstall curl wget sudo socat htop iftop unzip tar tmux ffmpeg btop ranger ncdu fzf vim nano git
			  ;;


		  33)
			  clearremove htop iftop tmux ffmpeg btop ranger ncdu fzf cmatrix sl bastet nsnake ninvaders vim nano git
			  opencode uninstall
			  rm -rf ~/.opencode
			  ;;
...
linux_bbr() {
	clearif [ -f "/etc/alpine-release" ]; then
		while true; do
			  clear
			  local congestion_algorithm=$(sysctl -n net.ipv4.tcp_congestion_control)
			  local queue_algorithm=$(sysctl -n net.core.default_qdisc)
...
docker_ssh_migration() {
	...
	# 备份
	# ----------------------------
	backup_docker() {echo -e "${gl_kjlan}正在备份 Docker 容器...${gl_bai}"
	...
	# 还原
	# ----------------------------
	restore_docker() {read -e -p  "请输入要还原的备份目录: " BACKUP_DIR
	...
	# 删除备份
	# ----------------------------
	delete_backup() {read -e -p  "请输入要删除的备份目录: " BACKUP_DIR
		[[ ! -d "$BACKUP_DIR" ]] && { echo -e "${gl_hong}备份目录不存在${gl_bai}"; return; }
		rm -rf "$BACKUP_DIR"
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  2)
			  ...
			  local volume_count=$(docker volume ls -q 2>/dev/null | wc -l)echo "Docker版本"
			  docker -v
		  ...
		  5)
			  while true; do
				  ...
				  case $sub_choice in
					  1)read -e -p "设置新网络名: " dockernetwork
						  docker network create $dockernetwork
						  ;;
					  2)read -e -p "加入网络名: " dockernetwork
						  ...
						  ;;
					  3)read -e -p "退出网络名: " dockernetwork
					  ...
					  4)read -e -p "请输入要删除的网络名: " dockernetwork
		  ...
		  6)
			  while true; do
				  ...
				  case $sub_choice in
					  ...
					  2)
					   ...
					   3)read -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定删除所有未使用的卷吗？(Y/N): ")" choice
						  case "$choice" in
							[Yy])
			  ...
			  ;;
		  7)
			  clearread -e -p "$(echo -e "${gl_huang}提示: ${gl_bai}将清理无用的镜像容器网络，包括停止的容器，确定清理吗？(Y/N): ")" choice
			  case "$choice" in
				[Yy])
			  ...
			  ;;
		  8)
			  clearbash <(curl -sSL https://linuxmirrors.cn/docker.sh)
			  ;;
		  ...
		  20)
			  clearread -e -p "$(echo -e "${gl_hong}注意: ${gl_bai}确定卸载docker环境吗？(Y/N): ")" choice
			  case "$choice" in
				[Yy])
...
linux_test() {
	...
	while true; do
	  ...
	  case $sub_choice in
		  1)
			  clearbash <(curl -Ls https://cdn.jsdelivr.net/gh/missuo/OpenAI-Checker/openai.sh)
			  ;;
		  2)
			  clearbash <(curl -L -s check.unlock.media)
			  ;;
		  3)
			  clearinstall wget
			  ...
			  ;;
		  4)
			  clearbash <(curl -Ls IP.Check.Place)
			  ;;
		  ...
		  11)
			  ...
			  ;;
		  12)
			  clearcurl ${gh_proxy}raw.githubusercontent.com/zhucaidan/mtr_trace/main/mtr_trace.sh | bash
			  ;;
		  13)
			  clearbash <(curl -Lso- https://git.io/superspeed_uxh)
			  ;;
		  14)
			  clearcurl nxtrace.org/nt |bash
		  ...
		  16)
			  clearcurl ${gh_proxy}raw.githubusercontent.com/ludashi2020/backtrace/main/install.sh -sSf | sh
			  ;;

		  17)
			  clearbash <(curl -sL ${gh_proxy}raw.githubusercontent.com/i-abc/Speedtest/main/speedtest.sh)
			  ;;

		  18)
			  clearbash <(curl -sL Net.Check.Place)
			  ;;
...
linux_Oracle() {
	 ...
	 while true; do
	  ...
	  case $sub_choice in
		  ...
		  4)
			  clearbash <(wget -qO- ${gh_proxy}github.com/Yohann0617/oci-helper/releases/latest/download/sh_oci-helper_install.sh)
			  ;;
		  5)
			  clear
			  add_sshpasswd
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  clear
	  # Discuz论坛
	  webname="Discuz论坛"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/discuz.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  mkdir $yuming
	  cd $yuming
	  wget -O latest.zip ${gh_proxy}github.com/openclaw-AI-make/ks-sh/Website_source_code/raw/main/Discuz_X3.5_SC_UTF8_20250901.zip
	  unzip latest.zip
	  rm latest.zip
	  ...
	  clear
	  # 可道云桌面
	  webname="可道云桌面"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/kdy.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  clear
	  # 苹果CMS
	  webname="苹果CMS"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/maccms.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  cd /home/web/html/$yuming/template/ && wget ${gh_proxy}github.com/openclaw-AI-make/ks-sh/Website_source_code/raw/main/DYXS2.zip && unzip DYXS2.zip && rm /home/web/html/$yuming/template/DYXS2.zip
	  cp /home/web/html/$yuming/template/DYXS2/asset/admin/Dyxs2.php /home/web/html/$yuming/application/admin/controller
	  cp /home/web/html/$yuming/template/DYXS2/asset/admin/dycms.html /home/web/html/$yuming/application/admin/view/system
	  mv /home/web/html/$yuming/admin.php /home/web/html/$yuming/vip.php && wget -O /home/web/html/$yuming/application/extra/maccms.php ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/Website_source_code/main/maccms.php
	  ...
	  clear
	  # 独脚数卡
	  webname="独脚数卡"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/dujiaoka.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  clear
	  # flarum论坛
	  webname="flarum论坛"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/flarum.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  clear
	  # typecho
	  webname="typecho"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/typecho.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  clear
	  # LinkStack
	  webname="LinkStack"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/refs/heads/main/index_php.conf
	  ...
	  20)
	  clear
	  webname="PHP动态站点"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  ldnmp_install_status
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/index_php.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  22)
	  clear
	  webname="站点重定向"echo "开始部署 $webname"
	  add_yuming
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/rewrite.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  24)
	  clear
	  webname="反向代理-域名"echo "开始部署 $webname"
	  add_yuming
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/reverse-proxy-domain.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  25)
	  clear
	  webname="Bitwarden"echo "开始部署 $webname"
	  add_yuming
	  ...
	  26)
	  clear
	  webname="halo"echo "开始部署 $webname"
	  add_yuming
	  ...
	  27)
	  clear
	  webname="AI绘画提示词生成器"echo "开始部署 $webname"
	  add_yuming
	  nginx_install_status
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/nginx/main/html.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget ${gh_proxy}github.com/openclaw-AI-make/ks-sh/Website_source_code/raw/refs/heads/main/ai_prompt_generator.zip
	  unzip $(ls -t *.zip | head -n 1)
	  rm -f $(ls -t *.zip | head -n 1)
	  ...
	  30)
	  clear
	  webname="静态站点"echo "开始部署 $webname"
	  add_yuming
	  repeat_add_yuming
	  nginx_install_status
	...
	32)
	  clearlocal backup_filename="web_$(date +"%Y%m%d%H%M%S").tar.gz"
	...
	33)
	  clearread -e -p "输入远程服务器IP: " useip
	  read -e -p "输入远程服务器密码: " usepasswd

	  cd ~
	  wget -O ${useip}_beifen.sh ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/beifen.sh > /dev/null 2>&1
	...
	37)
	  ...
	  while true; do
		  ...
		  case $sub_choice in
			  ...
			  cp /home/web/docker-compose1.yml /home/web/docker-compose.ymlecho "更新${ldnmp_pods}完成"
	...
	38)
		root_useread -e -p "$(echo -e "${gl_hong}强烈建议：${gl_bai}先备份全部网站数据，再卸载LDNMP环境。确定删除所有网站数据吗？(Y/N): ")" choice
		case "$choice" in
		  [Yy])
			cd /home/web/
			docker compose down --rmi all
...
moltbot_menu() {
	local app_id="114"check_openclaw_update() {
...
PY
	...
	install_moltbot() {
		echo "开始安装 OpenClaw..."install git jq
	...
	start_bot() {
		echo "启动 OpenClaw..."start_gateway
		break_end
	}

	stop_bot() {
		echo "停止 OpenClaw..."tmux kill-session -t gateway > /dev/null 2>&1
		openclaw gateway stop
		break_end
	}

	view_logs() {
		echo "查看 OpenClaw 状态日志"openclaw status
...
EOF
	...
	add-openclaw-provider-interactive() {echo "=== 交互式添加 OpenClaw Provider (全量模型) ==="
...
openclaw_api_manage_list() {
	local config_file="${HOME}/.openclaw/openclaw.json"while IFS=$'\t' read -r rec_type idx name base_url model_count api_type latency_txt latency_level; do
		case "$rec_type" in
			MSG)
				echo "$idx"
				;;
...
sync-openclaw-provider-interactive() {
	local config_file="${HOME}/.openclaw/openclaw.json"if [ ! -f "$config_file" ]; then
...
PY
	...
	delete-openclaw-provider-interactive() {
		...
		if [ -z "$provider_name" ]; thenecho "❌ provider 名称不能为空"
			break_end
			return 1
		fi
...
PY
	...
	openclaw_api_manage_menu() {while true; do
			clear
			echo "======================================="
...
REPO
	...
	change_model() {local orange="#FF8C00"
...
openclaw_json_get_bool() {
	...
	change_tg_bot_code() {while true; do
			clear
			echo "========================================"
			echo "            机器人连接对接            "
			echo "========================================"
...
PY
	...
	openclaw_memory_backup_export() {local backup_root=$(openclaw_backup_root)
...
" "$workspaces_json" "$tmp_payload"
	...
	openclaw_memory_backup_import() {local archive_path=$(openclaw_read_import_path "还原记忆全量 (支持多智能体)")
...
if os.path.isdir(agents_root):
	...
	openclaw_project_backup_export() {local openclaw_root="${HOME}/.openclaw"
	...
	openclaw_project_backup_import() {local openclaw_root="${HOME}/.openclaw"
	...
	openclaw_backup_delete_file() {local backup_root backup_root_real user_input target_file target_path target_type
		backup_root=$(openclaw_backup_root)
...
')
	...
	openclaw_memory_select_sources() {
		...
		if [ "$OPENCLAW_MEMORY_USE_MIRROR" = "true" ]; then
			...
			OPENCLAW_MEMORY_GH_PROXY="https://gh.ks.pro/"
		else
			if [ "$hf_ok" = "ok" ]; then
				OPENCLAW_MEMORY_HF_BASE="https://huggingface.co"
			elif [ "$mirror_ok" = "ok" ]; then
	...
	openclaw_memory_menu() {while true; do
			clear
			echo "======================================="
			echo "OpenClaw 记忆管理"
			echo "======================================="
...
PY
	...
	openclaw_permission_apply_standard() {openclaw_permission_require_openclaw || return 1
	...
	openclaw_permission_apply_developer() {openclaw_permission_require_openclaw || return 1
		openclaw_permission_backup_current || true
		local failed=0
		openclaw config set tools.profile coding || failed=1
		openclaw_permission_unset_optional tools.byProvider || failed=1
	...
	openclaw_permission_apply_full() {openclaw_permission_require_openclaw || return 1
		openclaw_permission_backup_current || true
		local failed=0
	...
	openclaw_permission_restore_official_defaults() {openclaw_permission_require_openclaw || return 1
		openclaw_permission_backup_current || true
		local failed=0
	...
	openclaw_permission_run_audit() {openclaw_permission_require_openclaw || return 1
		openclaw security audit
	}

	openclaw_permission_menu() {while true; do
			clear
			echo "======================================="
			echo "OpenClaw 权限管理"
			echo "======================================="
...
if not agents: print("当前未配置任何多智能体。")
 ...
 for item in itertools.islice(agents, 8):
  ident_obj=item.get("identity") if isinstance(item.get("identity"),dict) else {}; identity=ident_obj.get("name") or item.get("identityName") or item.get("name") or "-"; emoji=item.get("identityEmoji") or ""; ws=item.get("workspace") or "-"; print("- 智能体ID: [1;36m%s[0m" % item.get("id","?")); print("  身份名称: %s %s" % (identity, emoji)); print("  工作目录: %s" % ws)' "$(openclaw_multiagent_agents_json)" "$(openclaw_multiagent_bindings_json)" "$(openclaw_multiagent_sessions_json)"
	...
	openclaw_multiagent_list_agents() {python3 -c 'import json,sys; agents=json.loads(sys.argv[1] or "[]");
...
for idx,item in enumerate(agents,1):
 print("%s. %s" % (idx, item.get("id","?"))); print("   workspace : %s" % item.get("workspace","-")); ident=(item.get("identityName") or "-") + ((" " + item.get("identityEmoji")) if item.get("identityEmoji") else ""); print("   identity  : %s" % ident.strip()); print("   model     : %s" % (item.get("model") or "-")); print("   bindings  : %s" % item.get("bindings",0)); print("   default   : %s" % ("yes" if item.get("isDefault") else "no"))' "$(openclaw_multiagent_agents_json)"
	...
	openclaw_multiagent_add_agent() {openclaw_multiagent_require_openclaw || return 1
	...
	openclaw_multiagent_delete_agent() {openclaw_multiagent_require_openclaw || return 1
	...
	openclaw_multiagent_list_bindings() {python3 -c 'import json,sys; bindings=json.loads(sys.argv[1] or "[]");
...
for idx,item in enumerate(bindings,1):
 bind=item.get("bind") or item.get("binding") or item.get("scope") or "-"; print("%s. agent=%s | bind=%s" % (idx, item.get("agentId","?"), bind))' "$(openclaw_multiagent_bindings_json)"
	...
	openclaw_multiagent_add_binding() {openclaw_multiagent_require_openclaw || return 1
	...
	openclaw_multiagent_remove_binding() {openclaw_multiagent_require_openclaw || return 1
		local agent_id bind_value confirm
		read -e -p "请输入智能体 ID: " agent_id
	...
	openclaw_multiagent_show_sessions() {python3 -c 'import json,sys; obj=json.loads(sys.argv[1] or "{}"); sessions=obj.get("sessions",[]) if isinstance(obj,dict) else [];
...
for item in sessions[:10]: print("%s | %s | %s" % (item.get("agentId","?"), item.get("key","-"), item.get("model") or "-"))' "$(openclaw_multiagent_sessions_json)"
	...
	openclaw_multiagent_health_check() {openclaw_multiagent_require_openclaw || return 1
		local config_file
		config_file=$(openclaw_multiagent_config_file)
...
print("路由绑定数=%s" % len(bindings)); print("✅ 多智能体健康检查完成")' "$(openclaw_multiagent_agents_json)" "$(openclaw_multiagent_bindings_json)"
	...
	openclaw_multiagent_menu() {while true; do
			clear
			echo "======================================="
...
openclaw_backup_restore_menu() {while true; do
			clear
			echo "======================================="
			echo "OpenClaw 备份与还原"
			echo "======================================="
	...
	update_moltbot() {
		echo "更新 OpenClaw..."install_node_and_tools
		git config --global url."${gh_proxy}github.com/".insteadOf ssh://git@github.com/
		git config --global url."${gh_proxy}github.com/".insteadOf git@github.com:
		npm install -g openclaw@latest
	...
	uninstall_moltbot() {
		echo "卸载 OpenClaw..."openclaw uninstall
	...
	nano_openclaw_json() {install nano
	...
	# 主菜单
	openclaw_webui_menu() {while true; do
			clear
			openclaw_show_webui_addr
			echo
			echo "1. 添加域名访问"
	...
	while true; do
		...
		case $choice in
			...
			11)openclaw onboard --install-daemon
				break_end
				;;
			12)openclaw doctor --fixif sync_openclaw_api_models; then
					start_gateway
				else
...
if [ ! -d apps/.git ]; then
	timeout 10s git clone ${gh_proxy}github.com/openclaw-AI-make/ks-sh/apps.git
else
	cd apps
	# git pull origin main > /dev/null 2>&1
	timeout 10s git pull ${gh_proxy}github.com/openclaw-AI-make/ks-sh/apps.git main > /dev/null 2>&1
fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方应用列表"
  	  echo -e "${gl_kjlan}想要让你的应用出现在这里？查看开发者指南: ${gl_huang}https://dev.ks.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  13|cloudreve)
		...
		docker_app_install() {
			...
			curl -o /home/docker/cloud/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/cloudreve-docker-compose.yml
	  ...
	  74|moontv)
		...
		docker_app_install() {
			...
			curl -o /home/docker/moontv/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/moontv-docker-compose.yml
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "设置${docker_name}的登录密钥（sk-开头字母和数字组合）如: sk-ks123456789: " app_passwd
	  ...
	  91|gitea)
		...
		docker_app_install() {
			...
			curl -o /home/docker/gitea/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/gitea-docker-compose.yml
	...
	94|gopeed)
	  ...
	  96|2fauth)
		...
		docker_app_install() {
			...
			curl -o /home/docker/2fauth/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/2fauth-docker-compose.yml
	...
	98|wgc)
	  ...
	  99|dsm)
		...
		docker_app_install() {
			...
			curl -o /home/docker/dsm/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/docker/main/dsm-docker-compose.yml
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  b)
	  	clearlocal backup_filename="app_$(date +"%Y%m%d%H%M%S").tar.gz"
	  	echo -e "${gl_kjlan}正在备份 $backup_filename ...${gl_bai}"
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  1)
			  clear
			  install tmux
			  local SESSION_NAME="work1"tmux_run

			  ;;
		  2)
			  clear
			  install tmux
			  local SESSION_NAME="work2"tmux_run
			  ;;
		  3)
			  clear
			  install tmux
			  local SESSION_NAME="work3"tmux_run
			  ;;
		  4)
			  clear
			  install tmux
			  local SESSION_NAME="work4"tmux_run
			  ;;
		  5)
			  clear
			  install tmux
			  local SESSION_NAME="work5"tmux_run
			  ;;
		  6)
			  clear
			  install tmux
			  local SESSION_NAME="work6"tmux_run
			  ;;
		  7)
			  clear
			  install tmux
			  local SESSION_NAME="work7"tmux_run
			  ;;
		  8)
			  clear
			  install tmux
			  local SESSION_NAME="work8"tmux_run
			  ;;
		  9)
			  clear
			  install tmux
			  local SESSION_NAME="work9"tmux_run
			  ;;
		  10)
			  clear
			  install tmux
			  local SESSION_NAME="work10"tmux_run
			  ;;
		  ...
		  21)
			while true; do
			  ...
			  fiecho -e "SSH常驻模式 ${tmux_sshd_status}"
			  ...
			  case "$gongzuoqu_del" in
				1)
			  	  install tmux
			  	  local SESSION_NAME="sshd"grep -q "tmux attach-session -t sshd" ~/.bashrc || echo -e "\n# 自动进入 tmux 会话\nif [[ -z \"\$TMUX\" ]]; then\n    tmux attach-session -t sshd || tmux new-session -s sshd\nfi" >> ~/.bashrc
				  source ~/.bashrc
			  	  tmux_run
				  ;;
				2)
...
log_menu() {show_log_overview() {
	...
	while true; do
		...
		case $choice in
			1)read -erp "查看最近多少行日志？[默认 100]: " lines
				lines=${lines:-100}
				journalctl -n "$lines" --no-pager
				read -erp "按回车继续..."
				;;
			2)read -erp "请输入服务名（如 sshd、nginx）: " svc
...
env_menu() {
	...
	BASHRC="$HOME/.bashrc"
	PROFILE="$HOME/.profile"show_env_vars() {
		clearecho "========== 当前已生效环境变量（节选） =========="
		printf "%-20s %s\n" "变量名" "值"
		echo "-----------------------------------------------"
	...
	edit_file() {
		local file="$1"install nano
		nano "$file"
	}

	source_files() {
		echo "正在重新加载环境变量..."source "$BASHRC"
		source "$PROFILE"
		echo "✔ 环境变量已重新加载"
		read -erp "按回车继续..."
	}
...
create_user_with_sshkey() {
	...
	case "$sshkey_vl" in
		http://*|https://*)fetch_remote_ssh_keys "$sshkey_vl" "/home/$new_username"
			;;
		ssh-rsa*|ssh-ed25519*|ssh-ecdsa*)import_sshkey "$sshkey_vl" "/home/$new_username"
			;;
		*)
	  ...
	  case $sub_choice in
		  1)
			  while true; do
				  ...
				  find /usr/local/bin/ -type l -exec bash -c 'test "$(readlink -f {})" = "/usr/local/bin/ks" && rm -f {}' \;
				  if [ "$kuaijiejian" != "k" ]; then
					  ln -sf /usr/local/bin/ks /usr/local/bin/$kuaijiejian
				  fi
				  ln -sf /usr/local/bin/ks /usr/bin/$kuaijiejian > /dev/null 2>&1
				  echo "快捷键已设置"break_end
				  linux_Settings
			  done
			  ;;
...
EOF
		  ...
		  6)
			root_usewhile true; do
				...
				if [[ $new_port =~ ^[0-9]+$ ]]; then  # 检查输入是否为数字
					if [[ $new_port -ge 1 && $new_port -le 65535 ]]; thennew_ssh_port $new_port
					elif [[ $new_port -eq 0 ]]; thenbreak
					else
						echo "端口号无效，请输入1到65535之间的数字。"break_end
					fi
				else
					echo "输入无效，请输入数字。"break_end
				fi
			done
		  ...
		  8)
			...
			dd_xitong
			  ;;
		  9)
			root_useread -e -p "请输入新用户名（输入0退出）: " new_username
...
EOF
		  ...
		  root_usewhile true; do
			  ...
			  if [ -n "$new_hostname" ] && [ "$new_hostname" != "0" ]; then
				  ...
				  echo "主机名已更改为: $new_hostname"sleep 1
			  else
				  echo "已退出，未更改主机名。"
				  break
			  fi
		  ...
		  case $choice in
			  1)bash <(curl -sSL https://linuxmirrors.cn/main.sh)
				  ;;
			  2)bash <(curl -sSL https://linuxmirrors.cn/main.sh) --edu
				  ;;
			  3)bash <(curl -sSL https://linuxmirrors.cn/main.sh) --abroad
				  ;;
			  4)switch_mirror false false
				  ;;
		  ...
		  21)
			  root_usewhile true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "请输入新的解析记录 格式: 110.25.5.33 ks.pro : " addhost
			...
			root_usewhile true; do
				...
				case "$Limiting" in
				  1)
					...
					cd ~
					curl -Ss -o ~/Limiting_Shut_down.sh ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/Limiting_Shut_down1.sh
			  ...
			  case "$choice" in
				[Yy])cd ~
				  ...
				  if [ -f ~/TG-check-notify.sh ]; then
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/TG-check-notify.sh
					  chmod +x ~/TG-check-notify.sh
					  nano ~/TG-check-notify.sh
				  fi
				  ...
				  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/TG-SSH-check-notify.sh > /dev/null 2>&1
		  ...
		  26)
			  root_usecd ~
			  curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/upgrade_openssh9.8p1.sh
		  ...
		  61)
			clearecho "访问科技lion官方留言板，您对脚本有任何想法欢迎留言交流！"
			echo "https://board.ks.pro"
			echo "公共密码: ks.sh"
			  ;;

		  66)

			  root_useecho "一条龙系统调优"
			  echo "------------------------------------------------"
			  echo "将对以下内容进行操作与优化"
		  ...
		  100)
			...
			while true; do
			  clear
			  if grep -q '^ENABLE_STATS="true"' /usr/local/bin/ks > /dev/null 2>&1; then
			  	local status_message="${gl_lv}正在采集数据${gl_bai}"
			  elif grep -q '^ENABLE_STATS="false"' /usr/local/bin/ks > /dev/null 2>&1; then
			  ...
			  case $sub_choice in
				  1)
					  cd ~
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' /usr/local/bin/ks
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' ~/ks.sh
					  echo "已开启采集";;
				  2)
					  cd ~
					  sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' /usr/local/bin/ks
					  sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/ks.sh
					  echo "已关闭采集";;
				  *)
					  break
					  ;;
		  ...
		  102)
			  clearecho "卸载科技lion脚本"
			  echo "------------------------------------------------"
			  echo "将彻底卸载ks脚本，不影响你其他功能"
			  read -e -p "确定继续吗？(Y/N): " choice
	...
	root_usewhile true; do
		...
		case "$Limiting" in
			...
			23) # 移动文件或目录
				...
				if [ ! -e "$src_path" ]; then
					echo "错误: 文件或目录不存在。"continue
				fi

				read -e -p "请输入目标路径 (包括新文件名或目录名): " dest_path
				if [ -z "$dest_path" ]; then
					echo "错误: 请输入目标路径。"continue
				fi
...
cluster_python3() {
	...
	cd ~/cluster/
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/python-for-vps/main/cluster/$py_task
...
while true; do
	  ...
	  case $sub_choice in
		  1)read -e -p "服务器名称: " server_name
			  ...
			  ;;
		  2)read -e -p "请输入需要删除的关键字: " rmserver
		  ...
		  4)
			  clearecho -e "请将 ${gl_huang}/root/cluster/servers.py${gl_bai} 文件下载，完成备份！"
			  break_end
			  ;;

		  5)
			  clearecho "请上传您的servers.py，按任意键开始上传！"
			  echo -e "请上传您的 ${gl_huang}servers.py${gl_bai} 文件到 ${gl_huang}/root/cluster/${gl_bai} 完成还原！"
			  break_end
			  ;;

		  11)
			  local py_task="install_ks.py"
			  cluster_python3
			  ;;
		  12)
		  ...
		  51)read -e -p "请输入批量执行的命令: " mingling
...
echo -e "${gl_bai}网址: https://ks.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "域名优惠"
echo "------------------------"
...
echo -e "科技lion周边"
echo "------------------------"
echo -e "${gl_kjlan}官网: ${gl_bai}https://ks.pro/              ${gl_kjlan}导航: ${gl_bai}https://dh.ks.pro/${gl_bai}"
echo -e "${gl_kjlan}博客: ${gl_bai}https://blog.ks.pro/         ${gl_kjlan}软件中心: ${gl_bai}https://app.ks.pro/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://raw.githubusercontent.com            ${gl_kjlan}GitHub地址: ${gl_bai}${gh_https_url}github.com/openclaw-AI-make/ks-sh/sh${gl_bai}"
echo "------------------------"
echo ""
}
...
games_server_tools() {
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  1); cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/palworld.sh ; chmod +x palworld.sh ; ./palworld.sh
			 exit
			 ;;
		  2); cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/mc.sh ; chmod +x mc.sh ; ./mc.sh
			 exit
			 ;;
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks_sh_log.txt"
	echo "------------------------"

	curl -s ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks_sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)

	if [ "$sh_v" = "$sh_v_new" ]; then
		echo -e "${gl_lv}你已经是最新版本！${gl_huang}v$sh_v${gl_bai}"else
		echo "发现新版本！"
		echo -e "当前版本 v$sh_v        最新版本 ${gl_huang}v$sh_v_new${gl_bai}"
	fi


	local cron_job="ks.sh"
	local existing_cron=$(crontab -l 2>/dev/null | grep -F "$cron_job")
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/cn/ks.sh && chmod +x ks.sh
			else
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks.sh && chmod +x ks.sh
			fi
			canshu_v6
			CheckFirstRun_true
			yinsiyuanquan2
			cp -f ~/ks.sh /usr/local/bin/ks > /dev/null 2>&1
			echo -e "${gl_lv}脚本已更新到最新版本！${gl_huang}v$sh_v_new${gl_bai}"break_end
			~/ks.sh
			exit
			;;
		2)
			...
			if [ "$country" = "CN" ]; then
				SH_Update_task="curl -sS -O https://gh.ks.pro/raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks.sh && chmod +x ks.sh && sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ./ks.sh"
			elif [ -n "$ipv6_address" ]; then
				SH_Update_task="curl -sS -O https://gh.ks.pro/raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks.sh && chmod +x ks.sh && sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ./ks.sh"
			else
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/openclaw-AI-make/ks-sh/sh/main/ks.sh && chmod +x ks.sh"
			fi
			check_crontab_installed
			(crontab -l | grep -v "ks.sh") | crontab -
			...
			echo -e "${gl_lv}自动更新已开启，每天凌晨2点脚本会自动更新！${gl_bai}"break_end
			;;
		3)
			clear
			(crontab -l | grep -v "ks.sh") | crontab -
			echo -e "${gl_lv}自动更新已关闭${gl_bai}"break_end
			;;
		*)
			ks_sh
			;;
...
if [ "$#" -eq 0 ]; then
	...
	case $1 in
		...
		sshkey)
			...
			case "$1" in
				"" )
					# sshkey → 交互菜单sshkey_panel
					;;
				github )
					shiftfetch_github_ssh_keys "$1"
					;;
